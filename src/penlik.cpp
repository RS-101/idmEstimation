...
struct PenlikModelDataCaseSpecific {
    // ========================================================================
    ...
    PenlikModelDataCaseSpecific() {}

    // Constructor from List (already exists)
    PenlikModelDataCaseSpecific(const List& x) {
        // ====================================================================
        // REQUIRED FIELDS
        // ====================================================================
...
struct PenlikModelData {
    PenlikModelDataCaseSpecific case_1;
    PenlikModelDataCaseSpecific case_2;
    PenlikModelDataCaseSpecific case_3;
    PenlikModelDataCaseSpecific case_4;
    arma::mat penalty_matrix_12;
    arma::mat penalty_matrix_13;
    arma::mat penalty_matrix_23;

    // Constructor taking all data at once
    PenlikModelData(const List& data) {
        // Initialize case-specific data
        if (data.containsElementNamed("case_1")) {
            case_1 = PenlikModelDataCaseSpecific(as<List>(data["case_1"]));
        }
        if (data.containsElementNamed("case_2")) {
            case_2 = PenlikModelDataCaseSpecific(as<List>(data["case_2"]));
        }
        if (data.containsElementNamed("case_3")) {
            case_3 = PenlikModelDataCaseSpecific(as<List>(data["case_3"]));
        }
        if (data.containsElementNamed("case_4")) {
            case_4 = PenlikModelDataCaseSpecific(as<List>(data["case_4"]));
        }
...
SEXP create_penlik_model_data(List data) {
    PenlikModelData* md = new PenlikModelData(data);
    Rcpp::XPtr<PenlikModelData> p(md, true);
    return p;
}

double calc_case_1_log_likelihood(
    const PenlikModelDataCaseSpecific& md,
...
double calc_log_likelihood(
    ...
    Rcpp::XPtr<PenlikModelData> pointer(md_ptr);
    const PenlikModelData& mddata = *pointer;
...
double calc_full_penalty(
    ...
    double penalty_12 = kappa_12 * arma::as_scalar(
        theta_12.t() * mddata.penalty_matrix_12 * theta_12);
    double penalty_13 = kappa_13 * arma::as_scalar(
        theta_13.t() * mddata.penalty_matrix_13 * theta_13);
    double penalty_23 = kappa_23 * arma::as_scalar(
        theta_23.t() * mddata.penalty_matrix_23 * theta_23);
...
List calc_penalized_log_likelihood(
    ...
    double log_likelihood = calc_log_likelihood(
        md_ptr, theta_12, theta_13, theta_23);
