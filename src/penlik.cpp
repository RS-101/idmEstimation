...
double calc_case_1_log_likelihood(
    ...
    arma::vec integral = arma::cumsum(mid * md.dx_grid_T_obs);

    // Missing 0 insert

    arma::vec term_2 = factored_out %
        (integral.elem(arma::conv_to<arma::uvec>::from(
            md.T_obs_i_spline_mat_12 * theta_12)) -
            integral.elem(arma::conv_to<arma::uvec>::from(
            md.V_healthy_i_spline_mat_12 * theta_12)));
